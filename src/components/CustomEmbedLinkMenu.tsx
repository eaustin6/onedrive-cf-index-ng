          ...
          >
            <div className="inline-block max-h-[80vh] w-full max-w-3xl transform overflow-hidden overflow-y-scroll rounded border border-gray-400/30 bg-white p-4 text-left align-middle text-sm shadow-xl transition-all dark:bg-gray-900 dark:text-white">
              ...
              <div className="mt-4">
                ...
                <LinkContainer
                  title={'Default'}
                  value={`https://speedy.tiararose.workers.dev/api/download?url=${getBaseUrl()}/api/raw?path=${readablePath}${hashedToken ? `&odpt=${hashedToken}` : ''}`}
                />
                <LinkContainer
                  title={'URL encoded'}
                  value={`https://speedy.tiararose.workers.dev/api/download?url=${getBaseUrl()}/api/raw?path=${path}${hashedToken ? `&odpt=${hashedToken}` : ''}`}
                />
                <LinkContainer
                  title={'Customised'}
                  value={`https://speedy.tiararose.workers.dev/api/download?url=${getBaseUrl()}/api/name/${name}?path=${readablePath}${
                    hashedToken ? `&odpt=${hashedToken}` : ''
                  }`}
                />
                <LinkContainer
                  title={'Customised and encoded'}
                  value={`https://speedy.tiararose.workers.dev/api/download?url=${getBaseUrl()}/api/name/${name}?path=${path}${hashedToken ? `&odpt=${hashedToken}` : ''}`}
                />
              </div>
            </div>
          </Transition.Child>
